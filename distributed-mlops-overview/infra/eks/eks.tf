module "eks" {
  ...
  #---------------------------------------
  # Note: This can be further restricted to specific required for each Add-on and your application
  #---------------------------------------
  ...
  eks_managed_node_group_defaults = {
    iam_role_additional_policies = {
      ...
      # To enable CloudWatch plugin, see https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/install-CloudWatch-Observability-EKS-addon.html
...
# Allowes Nodes to perform SSH connection with each other. 
# Useful when scheduling MPI jobs using Training Operator or Volcano.
# For production purposes implement your own SG configurations
# or consider networking best practices - https://aws.github.io/aws-eks-best-practices/networking/index/
resource "aws_security_group_rule" "ssh_ingress_between_nodes" {
  type              = "ingress"
  from_port         = 22
  to_port           = 22
  protocol          = "tcp"
  ...
  description       = "DEMO ONLY: Allow SSH between nodes for MPI training jobs"
}
